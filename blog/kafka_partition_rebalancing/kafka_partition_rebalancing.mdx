---
slug: kafka_partition_rebalancing
title: Kafka Partition Rebalancing
authors: chien
date: 2024-03-24
tags: [ kafka,kafka_consumer,partition_rebalancing ]
---
...
There are two important rules of partition rebalancing
...
But it only has to guarantee those two rules, but also need to avoid the
partitions are not assigned evenly as much as possible. There are several ways
to implement a partition rebalancing algorithm. Let's go through them one by one
### Range

![range](./range.png)
[Range](https://kafka.apache.org/0110/javadoc/org/apache/kafka/clients/consumer/RangeAssignor.html) is the default partition rebalancing algorithm of many `Kafka` client
library. It splits the partitions of each topic into ranges and each range
contains consecutive partitions and assign each range to the consumers. 

For example: in the figure above, T1_P1(topic 1, partition 1), T1_P2, T2_P1, and
T2_P2 are assigned to C1 (consumer 1) whereas T1_P3, and T2_P3 are assigned to
C2. You can see that this algorithm can ensure evenly distribution in 1
topic but across the topics the distribution is not evenly
### Round Robin
...
For example: in the figure above, T1_P1, T1_P3, and T2_P2 are assigned to C1
whereas T1_P2, T2_P1, and T2_P3 are assigned to C2. Better than range algorithm,
round robin algorithm can ensure evenly distribution across all the topics
...
For example: in the figure above, after removing C3, T1_P1, and T2_P1 are still
assigned to C1, T1_P2, and T2_P2 are still assigned to C2. And C3's partitions
T1_P3 is assigned to C1, T2_P3 is assigned to C2
